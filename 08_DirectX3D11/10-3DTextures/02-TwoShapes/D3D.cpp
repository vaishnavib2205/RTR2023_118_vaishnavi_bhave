...
HRESULT initialize(void)
{
   ...
   -1.0f, -1.0f, +1.0f, // left-left
	...
	const float pyramidTexcoords[] =
	{
		...
		d3dBufferDesc.ByteWidth = sizeof(cube_position);
		...
		memcpy(d3dMappedSubresource.pData, cube_position, sizeof(cube_position));
	...
	// ************************************************************************************
		...
		// create vertex bufffer from above
		hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_colorBuffer_c);
		if (FAILED(hr))
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile, "gpID3D11Device->CreateBuffer color for cube function failed...");
			fclose(gpFile);
			return(hr);
		}
		else
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile, "gpID3D11Device->CreateBuffer color for cube function succeded...");
			fclose(gpFile);
		}
		// we will set this buffer in display because its dynamic
	...
	// *************************************************************************************************************************
		...
		// create vertex bufffer from above
		hr = gpID3D11Device->CreateBuffer(&d3dBufferDesc, NULL, &gpID3D11Buffer_colorBuffer_p);
		if (FAILED(hr))
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile, "gpID3D11Device->CreateBuffer color for pyramid function failed...");
			fclose(gpFile);
			return(hr);
		}
		else
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile, "gpID3D11Device->CreateBuffer color for pyramid function succeded...");
			fclose(gpFile);
		}
		// we will set this buffer in display because its dynamic
	...
	hr = loadD3DTexture(L"Vijay_Kundali.bmp", &gpID3D11ShaderResourceView_c);

	if (FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "loadD3DTexture function failed for kundali...\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "loadD3DTexture function succeded for kundali...\n");
		fclose(gpFile);
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "loadD3DTexture function failed for stone...\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "loadD3DTexture function succeded for stone...\n");
		fclose(gpFile);
