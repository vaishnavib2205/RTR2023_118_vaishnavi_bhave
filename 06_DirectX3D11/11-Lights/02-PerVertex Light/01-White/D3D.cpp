...
HRESULT initialize(void)
{
	...
	"float3 lightDirection = normalize((float3)(lightPosition) - eyeCoordinates.xyz ); \n " \
	...
	"float3 diffuseLight = lightDiffuse * materialDiffuse * max(dot(lightDirection , transformedNormals) , 0.0f); \n " \
	"float3 specularLight = lightSpecular * materialSpecular * pow(max(dot(reflectionVector , viewerVector) , 0.0f) , materialShininess); \n " \
	"output.phong_ads_light = ambientLight + diffuseLight + specularLight;  \n" \
	"}  \n" \
	"else  \n" \
	"{  \n" \
...
void display(void)
{
	...
	// parallel to vao_normal binding in OpenGL
