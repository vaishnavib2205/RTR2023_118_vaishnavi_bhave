...
void printD3Dinfo(void) {
	...
	IDXGIAdapter* pIDXGIAdaper = NULL;
	...
	if (FAILED(hr)) {
		...
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateDXGIFactory function Succeded...\n");
		fclose(gpFile);
	}

	if (pIDXGIFactory->EnumAdapters(0, &pIDXGIAdaper) != DXGI_ERROR_NOT_FOUND) {
		ZeroMemory((void*)&dxgiAdapterDesc, sizeof(DXGI_ADAPTER_DESC));
		pIDXGIAdaper->GetDesc(&dxgiAdapterDesc);
		WideCharToMultiByte(CP_ACP, 0, dxgiAdapterDesc.Description, 256, str, 255, NULL, NULL);
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "Graphic Card Nmae : %s\n", &str);
