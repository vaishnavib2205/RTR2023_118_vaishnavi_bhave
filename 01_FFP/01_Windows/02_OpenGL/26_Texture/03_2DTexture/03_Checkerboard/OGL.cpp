...
void loadGlTexture(void) {
	...
	//func call
	makeCheckImage(); // check image array gets filled in this function
	//create OpenGL texture obj
	glGenTextures(1, &texture_checkerboard);

	//Bind to the generated texture 
	glBindTexture(GL_TEXTURE_2D, texture_checkerboard); //binding point opengl madhe state machine ne define kelela asto

	//decide image Pixel alignment and unpacking
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // load image ne that is os ne mem madhe tila smjnarya bhashet img la load kela opengl mhnty mi ti image baghto aani packed data asel tar unpack karto data karan opengl la data unpack lagto aani te 4 chya slot mdhe unpack karto 

	//set texture parameters 
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);// image wrap kartoy// x axis
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_REPEAT); // t mhnje y axis
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); // mala tuzya texture chya int param de /////magnification////
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	
	//glgenerate mipmap la dyaychi garaj nahiye karan aplyakde image nahiye 
	//create multiple mipmap images 
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, CHECK_IMAGE_WIDTH, CHECK_IMAGE_HEIGHT, 0, GL_RGBA, GL_UNSIGNED_BYTE, (void*)checkImage); //arrray cha naav base address checkImage
...
void makeCheckImage(void) 
{
	...
	for (i = 0; i < CHECK_IMAGE_HEIGHT; i++)
	{
		...
		{
			c = (((i & 0x8) == 0) ^ ((j & 0x8) == 0)) * 255; //table varti tally karycha
