...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		case 49:
		case VK_NUMPAD1:
			cBuffer.giKeyPress = 1;
			
			break;

		case 50:
		case VK_NUMPAD2:
			cBuffer.giKeyPress = 2;

			break;

		case 51:
		case VK_NUMPAD3:
			cBuffer.giKeyPress = 3;

			break;

		case 52:
		case VK_NUMPAD4:
			cBuffer.giKeyPress = 4;

			break;

		default:
			cBuffer.giKeyPress = 0;
...
HRESULT initialize(void)
{
	...
	const float cube_postition[] = {
		...
		d3dBufferDesc.ByteWidth = sizeof(cube_postition);
	...
	// ************************************************************************************
		...
		ZeroMemory((void*)&d3dBufferDesc, sizeof(d3dBufferDesc));
		d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
		d3dBufferDesc.ByteWidth = 6 * 2 * sizeof(float);
		d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
...
void display(void)
{
	...
	if (cBuffer.giKeyPress == 1)
	{
		...
		squareTexcoord[8] = 1.0f;
		squareTexcoord[9] = 1.0f;
		squareTexcoord[10] = 1.0f;
		squareTexcoord[11] = 0.0f;
		
		
	}
	else if (cBuffer.giKeyPress == 2)
	{
		squareTexcoord[0] = 0.0f;
		...
		squareTexcoord[6] = 0.0f;
		squareTexcoord[7] = 0.0f;
		squareTexcoord[8] = 0.5f;
		squareTexcoord[9] = 0.5f;
		squareTexcoord[10] = 0.5f;
		squareTexcoord[11] = 0.0f;
	}
	else if (cBuffer.giKeyPress == 3)
	{
		squareTexcoord[0] = 0.0f;
		...
		squareTexcoord[6] = 0.0f;
		squareTexcoord[7] = 0.0f;
		squareTexcoord[8] = 2.0f;
		squareTexcoord[9] = 2.0f;
		squareTexcoord[10] = 2.0f;
		squareTexcoord[11] = 0.0f;
	}
	else if (cBuffer.giKeyPress == 4)
	{
	...
	// now copy actual data
	memcpy(d3dMappedSubresource.pData, squareTexcoord, sizeof(squareTexcoord));
