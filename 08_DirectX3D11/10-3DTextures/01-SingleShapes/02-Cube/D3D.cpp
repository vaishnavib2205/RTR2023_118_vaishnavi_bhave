...
ID3D11Buffer *gpID3D11Buffer_TexcoordBuffer = NULL;   // analogous to vbo_color.
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	d3dInputElementDesc[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;   //similar to glpixelstore(unpack_lignment) call in OpenGL
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// load the texture
	hr = loadD3DTexture(L"Vijay_Kundali.bmp" , &gpID3D11ShaderResourceView );
	if(FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile , "loadD3DTexture is Success !!! \n");
		fclose(gpFile);
	}
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile , "CreateSamplerState is Success !!! \n");
		fclose(gpFile);
	}
...
HRESULT loadD3DTexture(const wchar_t *textureFilename , ID3D11ShaderResourceView **ppID3D11ShaderResourceView)
{
	...
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile , "CreateWICTextureFromFile is Success !!! \n");
		fclose(gpFile);
	}
...
void display(void)
{
	...
	rotationMatrixX = XMMatrixRotationX(XMConvertToRadians(angle_Triangle));  // if angle sign (+) then rotation in left to right , and vice versa for (-)
	rotationMatrixY = XMMatrixRotationY(XMConvertToRadians(angle_Triangle));  // if angle sign (+) then rotation in left to right , and vice versa for (-)
	rotationMatrixZ = XMMatrixRotationZ(XMConvertToRadians(angle_Triangle));  // if angle sign (+) then rotation in left to right , and vice versa for (-)


	worldMatrix= rotationMatrixX * rotationMatrixY * rotationMatrixZ * translationMatrix;
	...
	// parallel to vao_texcoord binding in OpenGL
	stride = sizeof(float) * 2;
	offSet = 0  ;
	
	gpID3D11DeviceContext->IASetVertexBuffers(1 , 1 , &gpID3D11Buffer_TexcoordBuffer , &stride , &offSet);
	// set the texture shaderresource view in pixel shader
	...
	gpID3D11DeviceContext->Draw(12 , 6);
	gpID3D11DeviceContext->Draw(18 , 6);
	gpID3D11DeviceContext->Draw(24 , 6);
	gpID3D11DeviceContext->Draw(30 , 6);
