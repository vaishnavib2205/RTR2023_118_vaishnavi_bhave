...
HRESULT initialize(void)
{
	...
	d3dInputElementDesc[1].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;// SIMILAR TO PIXELSTOREI
	...
	1.0f, 0.0f, // bottom-right of front
		...
		d3dBufferDesc.ByteWidth = 6 * 3 *sizeof(float);
	...
	// ************************************************************************************
		...
		// create vertex bufffer for texcoord
		D3D11_MAPPED_SUBRESOURCE d3dMappedSubresource;

		ZeroMemory((void*)&d3dBufferDesc, sizeof(d3dBufferDesc));
		d3dBufferDesc.Usage = D3D11_USAGE_DYNAMIC;
		d3dBufferDesc.ByteWidth = sizeof(cube_texcoord);
		d3dBufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		d3dBufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		...
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile, "gpID3D11Device->CreateBuffer texcoord function failed...\n");
			fclose(gpFile);
			return(hr);
		}
		else
		{
			gpFile = fopen(gszLogFileName, "a+");
			fprintf(gpFile, "gpID3D11Device->CreateBuffer texcoord function succeded...\n");
			fclose(gpFile);
		}
		// we will set this buffer in display because its dynamic
		...
		memcpy(d3dMappedSubresource.pData, cube_texcoord, sizeof(cube_texcoord));
...
void makeCheckImage(void)
{
	...
	for (i = 0; i < CHECK_IMAGE_HEIGHT; i++)
	{
		for (j = 0; j < CHECK_IMAGE_WIDTH; j++)
		{
			...
			checkImage[i][j][0] = (int)c;
			checkImage[i][j][1] = (int)c;
			checkImage[i][j][2] = (int)c;
			checkImage[i][j][3] = (int)255;
		}
	}
}
HRESULT loadD3DTexture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	// Fill desc
	ZeroMemory((void*)&desc, sizeof(D3D11_TEXTURE2D_DESC));
	desc.Width = CHECK_IMAGE_WIDTH;
	desc.Height = CHECK_IMAGE_HEIGHT;
	desc.MipLevels = 1;
	desc.ArraySize = 1;
	...
	// Fill Subresource
	ZeroMemory((void*)&subResource, sizeof(D3D11_SUBRESOURCE_DATA));
	subResource.pSysMem = checkImage;
	subResource.SysMemPitch = 64 * 4;
	subResource.SysMemSlicePitch = 0;

	hr = gpID3D11Device->CreateTexture2D(&desc, &subResource, &texture);
	if (FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
	...
	hr = gpID3D11Device->CreateShaderResourceView(texture, pDesc, ppID3D11ShaderResourceView);
	if (FAILED(hr))
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateShaderResourceView function failed...\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		gpFile = fopen(gszLogFileName, "a+");
		fprintf(gpFile, "CreateShaderResourceView function succeded...\n");
		fclose(gpFile);
...
void display(void)
{
	...
	const float cube_postition[] = {
		// front
		-2.0f, +1.0f, 0.0f, // top-left of front
		0.0f, +1.0f, 0.0f, // top-right of front
		-2.0f, -1.0f, 0.0f, // bottom-left of front

		-2.0f, -1.0f, 0.0f, // bottom-left of front
		0.0f, +1.0f, 0.0f, // top-right of front
		0.0f, -1.0f, 0.0f, // bottom-right of front
	...
	// now copy actual data
	memcpy(d3dMappedSubresource.pData, cube_postition, sizeof(cube_postition));
	...
	// SET TEXCOORD buffer here
	Strid = sizeof(float) * 2;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(
		1,
		1,
	...
	// declared triangle geometry
	const float cube_postition2[] = {
		// front
		1.0f, 1.0f, 0.0f, // top-left of front
		2.41421f, +1.0f, 1.41421f, // top-right of front
		1.0f, -1.0f, 0.0f, // bottom-left of front

		1.0f, -1.0f, 0.0f, // bottom-left of front
		2.41421f, +1.0f, 1.41421f, // top-right of front
		2.41421f, -1.0f, 1.414210f, // 
	...
	// now copy actual data
	memcpy(d3dMappedSubresource.pData, cube_postition2, sizeof(cube_postition2));
