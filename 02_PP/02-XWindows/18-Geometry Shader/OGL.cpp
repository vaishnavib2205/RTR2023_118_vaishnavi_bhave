...
int initialize(void)
{
	...
	GLuint geomtryShaderObject = glCreateShader(GL_GEOMETRY_SHADER);

	glShaderSource(geomtryShaderObject, 1, (const GLchar**)&geometryShaderSourceCode, NULL);
	
	glCompileShader(geomtryShaderObject);
	...
	glGetShaderiv(geomtryShaderObject, GL_COMPILE_STATUS, &status);
	if (status == False)
	{
		glGetShaderiv(geomtryShaderObject, GL_INFO_LOG_LENGTH, &infoLogLength);
		if (infoLogLength > 0)
		{
			szInfoLog = (GLchar*)malloc(infoLogLength);
			if (szInfoLog != NULL)
			{
				glGetShaderInfoLog(geomtryShaderObject, infoLogLength, NULL, szInfoLog);
				fprintf(gpFile, "geomtryShaderObject Compilation Error Log : %s \n", szInfoLog);
				free(szInfoLog);
				szInfoLog = NULL;
			}
		}
	...
	glAttachShader(shaderProgramObject, geomtryShaderObject);
