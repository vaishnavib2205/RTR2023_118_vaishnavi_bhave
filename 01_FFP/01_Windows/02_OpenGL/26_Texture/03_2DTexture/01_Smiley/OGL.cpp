...
float textureScale[2] = {1.0f, 0.0f};
float textureOffset[2];
float textureRotAngle[3];
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CHAR:
		...
		case 'x':
			textureRotAngle[0] -= 0.1f;
			break;

		case 'y':
			textureRotAngle[1] -= 0.1f;
			break;
		case 'z':
			textureRotAngle[2] -= 0.1f;
			break;
...
void display(void)
{
	...
	//LoadIdentity for cube
	glLoadIdentity();

	glTranslatef(0.0, 0.0f, -5.0f);
	glScalef(1.0f, textureScale[1], 1.0f);
	...
	/*glTranslatef(0.0f, textureOffset[0], 0.0f);*/
	
	/*glScalef(textureScale[0], textureScale[0], 1.0f);*/
	/*glRotatef(textureRotAngle[0], 1.0f, 0.0f, 0.0f);
	glRotatef(textureRotAngle[1], 0.0f, 1.0f, 0.0f);
	glRotatef(textureRotAngle[2], 0.0f, 0.0f, 1.0f);*/
