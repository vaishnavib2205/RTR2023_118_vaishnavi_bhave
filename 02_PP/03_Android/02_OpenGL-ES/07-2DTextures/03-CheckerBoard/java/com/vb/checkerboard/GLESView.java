...
import com.vb.checkerboard.VertexAttributesEnum;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer,OnDoubleTapListener,OnGestureListener
{
    ...
    private final int CHECK_IMAGE_HEIGHT = 64; // as there are no MACROs in JAVA, we se them like this.
    ...
    private void initialize(GL10 gl)
    {
        ...
        final float[] square_texcoords = new float[]
        {
            1.0f , 1.0f,
            0.0f , 1.0f,
            0.0f , 0.0f,
        ...
        // 1. Allovate enough byte buffer
        ByteBuffer texture_Buffer = ByteBuffer.allocateDirect(square_texcoords.length * 4);
        ...
        // 4. now fill in the array into it
        textureBuffer.put(square_texcoords);

        // 5. now bring buffer to 0 after use
        textureBuffer.position(0);

        GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, square_texcoords.length * 4 , textureBuffer , GLES32.GL_STATIC_DRAW);
    ...
    private int loadGLTexture()
    {
        ...
        // create the bitmap from the data array given from above function 

        Bitmap bitmap = Bitmap.createBitmap(CHECK_IMAGE_HEIGHT , CHECK_IMAGE_HEIGHT , Bitmap.Config.ARGB_8888);
    ...
    {
        float[] square_position = new float[12];
        ...
        {    
            ...
            // 1. Allovate enough byte buffer
            ByteBuffer position_Buffer = ByteBuffer.allocateDirect(square_position.length * 4);

            // 2. set byte buffer order
            position_Buffer.order(ByteOrder.nativeOrder());

            // 3. treat this as float buffer
            FloatBuffer positionBuffer = position_Buffer.asFloatBuffer();
            ...
            GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, square_position.length * 4 , positionBuffer , GLES32.GL_DYNAMIC_DRAW);
            GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, 0);
