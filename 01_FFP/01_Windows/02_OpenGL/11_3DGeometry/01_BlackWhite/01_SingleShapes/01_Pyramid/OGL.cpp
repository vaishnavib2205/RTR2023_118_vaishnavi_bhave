...
int initialize(void)
{
	...
	//enabling depth
	glShadeModel(GL_SMOOTH); //beautification chi line aahe compulsory nahiye
	glClearDepth(1.0f);//framebuffer -> clearbuffer -> clearcolor cha call blue color ne clear kela jato tasach -> depthBuffer mdhlya sglya bits la 1 kar
	glEnable(GL_DEPTH_TEST);
	glDepthFunc(GL_LEQUAL);//depth sathi lagnara function les than or equal to z coordinates jyanche 1.0f-cleardepth
...
{
	// Code
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); //color buffer clear kar
