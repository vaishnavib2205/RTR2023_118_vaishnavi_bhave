...
//opencl related global variable declarations
...
int initialize(void)
{
	...
	//check opencl support if supported set default opencl device zero 
	cl_platform_id oclPlatformID;
	cl_uint dev_count;
	cl_device_id* oclDeviceIDs = NULL;

	//get opencl default platform id
	...
	{
		fprintf(gpFile, "There are no opencl supported device /n");
		uninitialized();
		exit(0);
	}
	else
	{
		...
		{
			fprintf(gpFile, "Memory Allocation failed for cl_device_id  /n");
			uninitialized();
			exit(0);
		}
	

	//fill the opencl device id array 
	oclResult = clGetDeviceIDs(oclPlatformID, CL_DEVICE_TYPE_GPU, dev_count, oclDeviceIDs, NULL);
	if (oclResult != CL_SUCCESS)
	{
		fprintf(gpFile, "clGetDeviceIDs function failed to get device count to fill opencl device count. \n");
		uninitialized();
		exit(0);
		...
		if (oclResult != CL_SUCCESS)
		{
			fprintf(gpFile, "clCreateCommandQueueWithProperties function failed. \n");
			uninitialized();
			exit(0);
		...
		// from above kernel code create opencl program
		...
		if(oclResult != CL_SUCCESS)
		{
			...
			oclBuildResult = clGetProgramBuildInfo(oclProgram , oclDeviceID , CL_PROGRAM_BUILD_LOG , sizeof(buffer) , buffer , NULL);
		...
		// now create opncl kernel

		oclKernel = clCreateKernel(oclProgram, "sinWaveKernel", NULL);
		if(oclResult != CL_SUCCESS)
		{
...
void display(void)
{
	...
	if (bOnGPU == TRUE)
	{
		...
		if(oclResult != CL_SUCCESS)
		{
			fprintf(gpFile , "clSetKernelArg for 0th arg failed \n");
			uninitialized();
			exit(0);
		}
		...
		if(oclResult != CL_SUCCESS)
		{
			fprintf(gpFile , "clSetKernelArg for 1st arg failed \n");
			uninitialized();
			exit(0);
		}
		...
		if(oclResult != CL_SUCCESS)
		{
			fprintf(gpFile , "clSetKernelArg for 2nd arg failed \n");
			uninitialized();
			exit(0);
		}
		...
		if(oclResult != CL_SUCCESS)
		{
			fprintf(gpFile , "clSetKernelArg for 3rd arg failed \n");
			uninitialized();
			exit(0);
		}
